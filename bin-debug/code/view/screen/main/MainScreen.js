...
(function (mx) {
    var MainScreen = (function (_super) {
        ...
        MainScreen.prototype.init_view = function () {
            ...
            for (var k in this.main_acty) {
                var acty_isopen = mx.MX_COMMON.MAIN_ACTYTIME[k];
                var arr_condition = acty_isopen.split("|");
                var isopen = false;
                switch (arr_condition.length) {
                    case 1://name  
                        if (acty_isopen == "open") {
                            isopen = true;
                        }
                        else if (acty_isopen == "close") {
                            isopen = false;
                        }
                        else if (gproxy[arr_condition[0]]) {
                            isopen = true;
                        }
                        break;
                    case 2://name|num 
                        if (Number(gproxy[arr_condition[0]]) < Number(arr_condition[1])) {
                            isopen = true;
                        }
                        break;
