...
var mx;
(function (mx) {
    var HeroItemRender = (function (_super) {
        __extends(HeroItemRender, _super);
        function HeroItemRender() {
            var _this = _super !== null && _super.apply(this, arguments) || this;
            _this.jinzhan = false;
            return _this;
        }
        HeroItemRender.prototype.init_render = function () {
            //this.addEventListener(egret.TouchEvent.TOUCH_TAP , this.click ,this);
            this.fg_flag = false;
            //this.eid = Tools.get_mn_res(this.data.mid,this.data.show ? "role" : "arole");
            this.eid = mx.Tools.get_mn_res(this.data.mid, "role");
            this.dataChanged();
        };
        HeroItemRender.prototype.on_remove = function (evt) {
            _super.prototype.on_remove.call(this);
            egret.Tween.removeTweens(this); //释放tween
            egret.Tween.removeTweens(this.mc_g); //释放tween
            // egret.Tween.removeTweens(this.die_p);//释放tween
            // this.buff_g.removeChildren();
            // this.debuff_g.removeChildren();
            if (this._timeout) {
        ...
        HeroItemRender.prototype.role_die = function () {
            var view = this;
            // "rotation" : -40
        ...
        HeroItemRender.prototype.changemc = function (evt) {
            //攻击后自己恢复正常
        ...
        //技能蓄力之后
        HeroItemRender.prototype.after_skill_launch = function () {
            var facade = mx.ApplicationFacade.getInstance();
            var info = this.round_data[this.get_round_data(this.data.id, "skill")].skill;
            ...
            if (this.jinzhan) {
                ...
                if (target_role) {
                    ...
                    var my_g = mx.Combat.stage["m" + (Math.abs(my_pos) + "_g")];
                    mx.Combat.stage.hero_g.addChild(my_g);
                    //调整层级
                    mx.Combat.stage.set_index("hero_g");
                    var distance_x_1 = 0;
                    if (Number(hero_id) * Number(target_id) < 0) {
                        distance_x_1 = target_g.x + 360 - my_g.x - 80;
                    }
                    if (distance_x_1 > 0) {
                        var distance_y = target_g.y - my_g.y;
                        // let target_g = Number(target_id) > 0 ? Combat.stage.hero_g : Combat.stage.enermy_g;
                        // let my_g = Number(this.data.id) > 0 ? Combat.stage.hero_g : Combat.stage.enermy_g;
                        // let p1 = target_g.localToGlobal(role_g.x,role_g.y);
                        // let p2 = my_g.globalToLocal(p1.x,p1.y);
                        ...
                        this._timeout = egret.setTimeout(function () {
                            ...
                            this._timeout = null;
                            this.hero_receivedamage('skill');
                        }, this, 7 * mx.FightView.FLASH_DESC); //长按0.75秒
                        ...
                        egret.Tween.get(this).to({ "x": distance_x_1, "y": distance_y }, 7 * mx.FightView.FLASH_DESC).call(function () {
                            //近战挥刀特效
                            ...
                            atk_ef1.scaleX = atk_ef1.scaleY = 2;
                            atk_ef1.name = ef_id;
        ...
        HeroItemRender.prototype.dataChanged = function () {
            var data = this.data;
            if (!data || !this.skin) {
                return;
            }
            ...
            if (data.show) {
                this.height = 150;
                this.removeChild(this.display_g);
                this.mc_g.scaleX = this.mc_g.scaleY = 0.8;
                var hapi = mx.ApiTool.getAPINode(mx.MX_APINAME.HERO, "id", this.data.mid);
                ...
                this.name_t.text = hapi.hero_name;
                this.show_g.visible = true;
            }
            else {
                this.show_g.visible = false;
                if (data.hp) {
        ...
        HeroItemRender.prototype.init_dragon = function (state) {
        ...
        /*----------外部接口-----------------*/
        //出手动画展示
        HeroItemRender.prototype.CommonAttack = function (data) {
            ...
            if (!info.extra && (Number(info.skill_id) > 0)) {
                ...
                if (!this.ef_g.getChildByName("skill_launch")) {
                    this.ef_g.addChild(atk_ef);
                }
            }
            else {
        ...
        HeroItemRender.prototype.BigAttack = function (data) {
            this.round_data = null;
            this.round_data = data; //Tools.arr2obj(data, "hero_id");
            //大招launch 
        ...
        HeroItemRender.prototype.continue_big_attack = function () {
            this.curr_state = mx.GameRole.ROLE_ATTACK1;
            this.mcDataFactory.animation.play(this.curr_state, 1);
            var id = this.data.id;
            var role = mx.Combat.stage._arr_hero[id];
        ...
        HeroItemRender.prototype.get_round_data = function (id, ziduan) {
            var view = this;
        ...
        HeroItemRender.prototype.hero_receivedamage = function (type1, middle) {
            ...
            var facade = mx.ApplicationFacade.getInstance();
            //找到出手记录
            var info = data[this.get_round_data(this.data.id, "skill")].skill;
            var skill_info = mx.FightTools.get_skill_info(info.skill_id);
        ...
        HeroItemRender.prototype.launch_over = function () {
            var facade = mx.ApplicationFacade.getInstance();
            var id = "big_launch";
        ...
        //血量变化
        HeroItemRender.prototype.set_blood = function (percent, value) {
        ...
        //能量变化
        HeroItemRender.prototype.set_mp = function (percent) {
        ...
        //敌方技能爆炸光效后
        HeroItemRender.prototype.after_point = function () {
        ...
        //受创动画
        HeroItemRender.prototype.on_hit = function (pointef, first, temp, from_id) {
            //默认受创
            var facade = mx.ApplicationFacade.getInstance();
            ...
            if (pointef == 'damage') {
                ...
                if (mx.Combat.stage.display_mode) {
                    ...
                    if (first && first.id) {
                        ...
                        var p_x = mx.Combat.stage["m" + role.fighter.pos_id + "_g"].left;
                        var p_y = mx.Combat.stage["m" + role.fighter.pos_id + "_g"].bottom;
                        var role_g = mx.Combat.stage.efs_g;
                        ...
                        atk_ef.scaleX = first.id == 'my_atk' ? -2 : 2;
        ...
        HeroItemRender.prototype.init_buff = function (info) {
            this.buff_info = info;
        };
        //buff生效光效
        HeroItemRender.prototype.buff_start = function () {
            var id = this.data.id;
            var role = mx.Combat.stage._arr_hero[id];
            if (role) {
                ...
                if (fighter.buff.length) {
                    if (fighter.buff[fighter.buff.length - 1]) {
                        var atk_ef = new mx.GeneralEffect(this.buff_info.ef + "_my");
        ...
        //播放效果
        HeroItemRender.prototype.show_effect = function (id, state, self) {
            var atk_ef = new mx.GeneralEffect(id);
            atk_ef.change_framerate(24, 1);
            atk_ef.horizontalCenter = 0;
            atk_ef.verticalCenter = 0;
            atk_ef.name = id;
            var group = self == 'self' ? mx.Combat.stage.hero_g : mx.Combat.stage.enermy_g;
            if (!group.getChildByName(id)) {
                group.addChild(atk_ef);
            }
        };
        HeroItemRender.prototype.fuhuo_ing = function () {
            var view = this;
        ...
        HeroItemRender.prototype.fuhuo_com = function () {
            var view = this;
            var buff_g = this.buff_g;
        ...
        HeroItemRender.prototype.add_buff = function (buff_info, buff) {
            //buff 还是debuff
            var view = this;
        ...
        HeroItemRender.prototype.buff_play = function (name) {
        ...
        HeroItemRender.prototype.remove_buff = function (buffid) {
            var view = this;
            var buff_g = this.buff_g;
        ...
        return HeroItemRender;
    }(mx.BasicRender));
    mx.HeroItemRender = HeroItemRender;
    __reflect(HeroItemRender.prototype, "mx.HeroItemRender");
})(mx || (mx = {}));
//# sourceMappingURL=HeroItemRender.js.map