...
(function (mx) {
    var FightTool = (function () {
        ...
        FightTool.prototype.cal_hero_prop = function (hero, type, dress, hero_arr, jiuzi) {
                ...
                for (var j_1 in hero.skills) {
                    ...
                    if (skill_info && skill_group && skill_info.type == 'passive') {
                        if (arr.indexOf(skill_info.skilltype) > -1) {
                            ...
                            var jiuzi_add = 0;
                            var pass_jiuzi = mx.ApiTool.getAPINode(mx.MX_APINAME.JIUZIKAIQI, "add_skill", skill.skill_id);
                            if (pass_jiuzi && jiuzi[Number(pass_jiuzi.jiuzi) - 1] >= 6) {
                                jiuzi_add = Number(pass_jiuzi.extra_add_num);
                            }
                            prop[skill_info.skilltype + "_pass"] += (Number(skill.skill_level) * Number(skill_group.Growth1Multiplier) * (Number(skill_group.Growth1Value) + jiuzi_add));
                        }
                    }
                }
            }
            ...
            var jiuzi_apis = mx.ApiTool.getAPINodes(mx.MX_APINAME.JIUZIKAIQI, "hero_id", hero.mid, "leixing", 2);
            var jiuzi_apis2 = mx.ApiTool.getAPINodes(mx.MX_APINAME.JIUZIKAIQI, "hero_id", hero.mid, "leixing", 3);
            for (var k_2 in jiuzi_apis) {
                if (jiuzi[Number(jiuzi_apis[k_2].jiuzi) - 1] >= 6) {
                    switch (jiuzi_apis[k_2].extra_add_type) {
                        case "HP":
                            hp_fate += Number(jiuzi_apis[k_2].extra_add_num) / 100;
                            break;
                        case "AD":
                            ad_fate += Number(jiuzi_apis[k_2].extra_add_num) / 100;
                            break;
                        case "ARM":
                            arm_fate += Number(jiuzi_apis[k_2].extra_add_num) / 100;
                            break;
                    }
                }
            }
            for (var k_3 in jiuzi_apis2) {
                if (jiuzi[Number(jiuzi_apis2[k_3].jiuzi) - 1] >= 6) {
                    if (!prop[jiuzi_apis2[k_3].extra_add_type + "_jiuzilv_add"]) {
                        prop[jiuzi_apis2[k_3].extra_add_type + "_jiuzilv_add"] = 0;
                    }
                    prop[jiuzi_apis2[k_3].extra_add_type + "_jiuzilv_add"] += Number(jiuzi_apis2[k_3].extra_add_num);
                }
            }
            ...
            //封印之力加成
            for (var i_3 in hero.fy_skill) {
                var unit = mx.ApiTool.getAPINode(mx.MX_APINAME.SOUL, "id", i_3);
                if (hero.fy_skill[i_3].level) {
                    var value1 = Number(unit.EffectValue1) + Number(unit.GrowthValue1) * (Number(hero.fy_skill[i_3].level) - 1);
                    switch (Number(i_3)) {
                        case 6:
        ...
        FightTool.prototype.cal_fight_power = function (hero, skip, dress, hero_arr, jiuzi) {
            ...
            if (hero.fy_skill && ((typeof hero.fy_skill.length != "undefined" && hero.fy_skill.length) || typeof hero.fy_skill.length == "undefined")) {
                var fy = mx.ApiTool.getAPINode(mx.MX_APINAME.SOULZLADD, 'hero_id', hero.mid);
                var xishu_1 = fy.jc.split('|');
                if (hero.fy_skill[xishu_1[0]]) {
                    zl += (big_skill_level * Number(hero.fy_skill[xishu_1[0]].level) * Number(xishu_1[1]));
                }
            }
